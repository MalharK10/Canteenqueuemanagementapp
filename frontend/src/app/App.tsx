...
export default function App() {
  ...
  useEffect(() => {
    fetch('http://65.2.183.102:5000/api/auth/me', { credentials: 'include' })
      .then(res => {
  ...
  const fetchMenu = useCallback(async () => {
    try {
      const res = await fetch('http://65.2.183.102:5000/api/menu', { credentials: 'include' });
      if (res.ok) {
  ...
  const fetchQueueInfo = useCallback(async () => {
    try {
      const res = await fetch('http://65.2.183.102:5000/api/orders/queue/info', { credentials: 'include' });
      if (res.ok) {
  ...
  const fetchActiveOrder = useCallback(async () => {
    try {
      const res = await fetch('http://65.2.183.102:5000/api/orders', { credentials: 'include' });
      if (res.ok) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const res = await fetch(`http://65.2.183.102:5000/api/orders/${currentOrder.id}`, { credentials: 'include' });
        if (res.ok) {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      const res = await fetch('http://65.2.183.102:5000/api/orders', {
        method: 'POST',
  ...
  if (isAdmin) {
    return <AdminDashboard onLogout={async () => {
      await fetch('http://65.2.183.102:5000/api/admin/logout', { method: 'POST', credentials: 'include' });
      setIsAdmin(false);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <Header
        ...
        onLogout={async () => {
          await fetch('http://65.2.183.102:5000/api/auth/logout', { method: 'POST', credentials: 'include' });
          setIsAuthenticated(false);
          setUserProfile(null);
          setAuthView('login');
          setCartItems([]);
