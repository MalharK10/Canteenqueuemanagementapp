...
**Request:**
```bash
curl http://localhost:3000/api/orders/queue/info \
  -H "Cookie: <your_auth_cookie>"
```
...
**Request:**
```bash
curl http://localhost:3000/api/orders/queue/all \
  -H "Cookie: <your_auth_cookie>"
```
...
**Request:**
```bash
curl http://localhost:3000/api/orders/queue/position \
  -H "Cookie: <your_auth_cookie>"
```
...
**Request:**
```bash
curl -X PATCH http://localhost:3000/api/orders/507f1f77bcf86cd799439011/status \
  -H "Content-Type: application/json" \
  -H "Cookie: <your_auth_cookie>" \
  -d '{
    "status": "preparing"
